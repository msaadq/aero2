    ...
    def __init__(self,table_name,divisions=11):
        ...
        for x in range(0,self.maxAirIndex,self.divisionInterval):
            ...
            if (self.table_name==self.database.RESULTS_TABLE_NAME):
                whereQuery=" air_index "+" >= "+str(x)+" AND "+"air_index <= "+str(x+self.divisionInterval-1)
            ...
            if tuples:
                ...
                response=self.uploader.upload(data, tileset='muddassir235.'+"Air"+self.table_name[6:]+str(itertator),
                ...
                newfile.write("{\"type\": \"FeatureCollection\", \"features\": [{  \"type\": \"Feature\", \"properties\": {}, \"geometry\": { \"type\": \"Point\", \"coordinates\": [26.103515625,-84.95930495623834]}}]}")
                newfile.close()
                data = open("Air"+self.table_name[6:]+str(itertator)+".geojson", 'rb')
                #and upload it to mapbox
                response=self.uploader.upload(data,tileset='muddassir235.'+"Air"+self.table_name[6:]+str(itertator),
